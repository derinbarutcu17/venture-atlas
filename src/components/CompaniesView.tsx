...
export const CompaniesView: React.FC<CompaniesViewProps> = ({ startups, searchTerm }) => {
    ...
    return (
        <div className="w-full h-full bg-[#f8fafc] text-[#0a0a0a] font-mono flex flex-col pt-8 px-12 overflow-hidden select-none">

            {/* Header Section */}
            <header className="mb-12 flex-shrink-0">
                <h1 className="text-[6vw] leading-[0.85] font-black tracking-tighter uppercase m-0 max-w-4xl">
                    Berlin<br />Ecosystem
                </h1>
            </header>

            {/* Main Typographic Grid */}
            <div className="flex-1 w-full grid grid-cols-4 gap-12 overflow-y-auto custom-scrollbar pb-24 pr-4">
                {groupedData.map((sector, index) => {
                    const idx = (index + 1).toString().padStart(2, '0');
                    return (
                        <div key={sector.name} className="flex flex-col relative h-max">
                            ...
                            <div className="flex flex-col space-y-3 w-full border-t border-black/10 pt-4 relative z-10">
                                {sector.verticals.map(vertical => {
                                    ...
                                    return (
                                        <div key={vertical.name} className="flex flex-col text-[10px] font-bold uppercase tracking-widest">
                                            ...
                                            {isExpanded && (
                                                <div className="mt-2 mb-4 pl-2 border-l border-black/20 flex flex-col space-y-1.5 overflow-y-auto max-h-[200px] custom-scrollbar">
                                                    {vertical.startups.map(s => (
                                                        <div key={s.id} className="text-[9px] font-medium opacity-70 hover:opacity-100 hover:text-black cursor-pointer truncate transition-opacity flex justify-between">
                                                            <span className="truncate pr-2">{s.name}</span>
                                                            <span className="tabular-nums opacity-50">€{s.funding >= 1000 ? (s.funding / 1000).toFixed(1) + 'B' : s.funding + 'M'}</span>
                                                        </div>
                                                    ))}
                                                </div>
                                            )}
            ...
            {/* Brutalist Footer */}
            <footer className="w-full flex justify-between items-end pb-8 pt-6 border-t border-black/10 uppercase text-[9px] font-bold tracking-widest text-[#0a0a0a]/60 bg-[#f8fafc] flex-shrink-0 relative z-20">
                <div className="flex flex-col space-y-4">
                    <div className="w-8 h-8 bg-black text-white flex items-center justify-center text-lg italic tracking-tighter font-black">B</div>
